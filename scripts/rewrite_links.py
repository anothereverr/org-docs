...
def _rewrite_wiki_links(
    ...
    Rules:
      ...
      - target matches a known page (case-insensitive) → {target}.md
      - target already ends with .md → normalise Home.md → index.md, else unchanged
      - unknown targets → unchanged + WARNING logged
    """
    ...
    def _replace(match: re.Match) -> str:
        ...
        # Check against known pages (case-insensitive)
        if target.lower() in known_lower:
            # Preserve original casing from the link target for the filename
            # (GitHub wikis are case-sensitive in filenames on Linux)
            return f"[{text}]({target}.md{anchor})"

        # Unknown target
        logger.warning(
            "[%s] Unresolved wiki link: [%s](%s) — leaving unchanged",
            slug,
            text,
            target,
        )
